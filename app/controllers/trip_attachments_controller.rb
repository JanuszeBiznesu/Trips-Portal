class TripAttachmentsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @trip_attachment.update(trip_attachment_params)
        format.html { redirect_to @trip_attachment.trip, notice: 'Post attachment was successfully updated.' }
      end 
    end
  end
  ...
  private
    ...
    def trip_attachment_params
      params.require(:trip_attachment).permit(:trip_id, :avatar)
    end
end
